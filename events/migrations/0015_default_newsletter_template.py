...
def reverse_default_template(apps, schema_editor):
    # This is the reverse function that will be called when the migration is reversed
    NewsletterTemplate = apps.get_model('events', 'NewsletterTemplate')
    # Delete the default template if it exists
    NewsletterTemplate.objects.filter(name="Default Welcome Template", is_default=True).delete()
