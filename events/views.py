...
from .models import Event, SubEvent, Review, Booking, UserProfile, ContactMessage, GalleryItem, SubEventCategory, CartItem, UserMessage, ActivityLog, Newsletter, NewsletterTemplate, NewsletterCampaign
...
def manager_send_newsletter(request):
    ...
    try:
            ...
            try:
                ...
                if template_id and template_id != '0':
                    try:
                        ...
                        messages.warning(request, f"Template not found: {str(e)}. Using content as is.")
                ...
                if send_test:
                    try:
                        ...
                        if hasattr(campaign, 'template') and campaign.template:
                ...
                try:
                    ...
                    try:
                        ...
                        print(f"Error logging activity: {str(log_error)}")
                    ...
                    try:
                        ...
                        print(f"Error saving campaign status: {str(save_error)}")
                    messages.error(request, f"Error sending newsletter: {str(e)}")
            except Exception as post_error:
                messages.error(request, f"Error processing form: {str(post_error)}")
        ...
        # Fallback to a very simple version if anything goes wrong
        messages.error(request, f"An error occurred: {str(e)}")
        return render(request, 'events/manager/send_newsletter.html', {
            'templates': [],
            'total_subscribers': 0,
            'active_subscribers': 0,
            'recent_subscribers': 0,
...
def admin_newsletters(request):
    ...
    context = {
        'subscribers': subscribers[:10],  # Show only the latest 10
        'templates': templates,
        'campaigns': campaigns[:10],  # Show only the latest 10
        'total_subscribers': total_subscribers,
        'active_subscribers': active_subscribers,
    ...
    return render(request, 'events/admin/newsletters.html', context)
...
def admin_newsletter_subscribers(request):
    ...
    search_query = request.GET.get('search')
    if search_query:
        subscribers = subscribers.filter(Q(email__icontains=search_query) | Q(name__icontains=search_query))
    ...
    return render(request, 'events/admin/newsletter_subscribers.html', context)


@login_required
@user_passes_test(lambda u: u.is_staff)
def admin_newsletter_templates(request):
    """Admin newsletter templates page"""
    templates = NewsletterTemplate.objects.all().order_by('-updated_at')
    ...
    return render(request, 'events/admin/newsletter_templates.html', context)


@login_required
@user_passes_test(lambda u: u.is_staff)
def admin_newsletter_template_edit(request, template_id=None):
    """Edit or create newsletter template"""
    if template_id:
        template = get_object_or_404(NewsletterTemplate, id=template_id)
        is_new = False
    else:
    ...
    if request.method == 'POST':
        ...
        messages.success(request, f"Template {'created' if is_new else 'updated'} successfully!")
        return redirect('admin_newsletter_templates')
    ...
    return render(request, 'events/admin/newsletter_template_edit.html', context)


@login_required
@user_passes_test(lambda u: u.is_staff)
def admin_newsletter_template_delete(request, template_id):
    ...
    return render(request, 'events/admin/newsletter_template_delete.html', context)


@login_required
@user_passes_test(lambda u: u.is_staff)
def admin_newsletter_campaigns(request):
    ...
    return render(request, 'events/admin/newsletter_campaigns.html', context)


@login_required
@user_passes_test(lambda u: u.is_staff)
def admin_newsletter_campaign_edit(request, campaign_id=None):
    """Edit or create newsletter campaign"""
    if campaign_id:
    ...
    if request.method == 'POST':
        ...
        messages.success(request, f"Campaign {'created' if is_new else 'updated'} successfully!")
        return redirect('admin_newsletter_campaigns')
    ...
    return render(request, 'events/admin/newsletter_campaign_edit.html', context)


@login_required
@user_passes_test(lambda u: u.is_staff)
def admin_newsletter_campaign_delete(request, campaign_id):
    ...
    return render(request, 'events/admin/newsletter_campaign_delete.html', context)


@login_required
@user_passes_test(lambda u: u.is_staff)
def admin_newsletter_campaign_send(request, campaign_id):
    """Send newsletter campaign"""
    campaign = get_object_or_404(NewsletterCampaign, id=campaign_id)

    if campaign.status not in ['draft', 'scheduled', 'failed']:
        messages.error(request, f"Cannot send campaign with status '{campaign.get_status_display()}'")
        return redirect('admin_newsletter_campaigns')
...
@login_required
@user_passes_test(lambda u: u.is_staff)
def admin_newsletter_campaign_preview(request, campaign_id):
    ...
    context = Context({
        ...
        'subject': campaign.subject,
        'custom_message': campaign.content,
        'unsubscribe_url': f"{request.scheme}://{request.get_host()}/newsletter/unsubscribe/?email={request.user.email}"
    })
